...
import { CredentialStore, Passport, nodeMsPassportAvailable } from "node-ms-passport";
 ...
 * the windows credential vault and protected using
 * CredProtectW. The random bytes are stored as the
 * user name encoded as a base64 string. The account
 * id consists of the app id and the vault id.
 *
...
export default class WindowsBiometricProvider implements BiometricProvider {
    ...
    public async supportsBiometricUnlock(): Promise<boolean> {
        return nodeMsPassportAvailable() && Passport.passportAvailable();
    }
    ...
    private static async getKeys(challenge: Buffer, initial: boolean = false): Promise<AesKeyIv> {
        ...
        const key = crypto.createHash(WindowsBiometricProvider.KEY_HASH_TYPE)
            .update(signature)
            .digest();
        const iv = crypto.createHash(WindowsBiometricProvider.IV_HASH_TYPE)
            .update(signature)
            .digest();
